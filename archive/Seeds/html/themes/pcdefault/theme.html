...
include("themes/pcdefault/tables.php");
...
function themeheader() {
    ...
    echo "<script language=\"Javascript\" src=\"themes/pcdefault/style/fade2.js\"></script>";
    if ($banners == 1) {
	include("banners.php");
    }
  ...
  $theuser = "&nbsp;&nbsp;Welcome $username!";
    ...
    $tmpl_file = "themes/pcdefault/header.html";
    ...
    print $r_file;
    blocks(left);
    $tmpl_file = "themes/pcdefault/left_center.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
...
function themefooter() {
    ...
    if ($index == 1) {
	$tmpl_file = "themes/pcdefault/center_right.html";
	$thefile = implode("", file($tmpl_file));
	$thefile = addslashes($thefile);
	$thefile = "\$r_file=\"".$thefile."\";";
	eval($thefile);
    ...
    $tmpl_file = "themes/pcdefault/footer.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
...
function themeindex ($aid, $informant, $time, $title, $counter, $topic, $thetext, $notes, $morelink, $topicname, $topicimage, $topictext) {
    ...
    $tmpl_file = "themes/pcdefault/story_home.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
...
function themearticle ($aid, $informant, $datetime, $title, $thetext, $topic, $topicname, $topicimage, $topictext) {
    ...
    $tmpl_file = "themes/pcdefault/story_page.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
...
function themesidebox($title, $content, $bid = -1) {
	...
	if ($bid == -1) {
		...
		$link_text = "<a href=\"$link_text\"><img src=\"themes/pcdefault/images/block_$img_text.gif\" border=\"0\" alt=\"$img_text\"></a>";
   }
   
    //Set width for side block
    $block_width = "150";
    $tmpl_file = "themes/pcdefault/blocks.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
    print $r_file;
		...
		if ($show || $bid == -1) {
	    $tmpl_file = "themes/pcdefault/blocks_content.html";
	    $thefile = implode("", file($tmpl_file));
	    $thefile = addslashes($thefile);
	    $thefile = "\$r_file=\"".$thefile."\";";
	    eval($thefile);
