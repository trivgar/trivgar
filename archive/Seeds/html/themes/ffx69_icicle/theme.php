...
/************************************************************/
/* Orginal Theme: Odyssey                                   */
/* Modifiziert: Helmut Theisen (http://www.ffx69.de)        */
/* Email: info@ffx69.de                                     */
/* More Themes from ffx69 on http://www.ffx69.de            */
/*                                                          */
/************************************************************/
...
include("themes/ffx69_icicle/tables.php");
...
function themeheader() {
    ...
    echo "<body bgcolor=\"#E9EEF3\" text=\"#000000\" link=\"#004080\" vlink=\"#004080\" alink=\"#004080\">";
    echo "<!-- Theme modifiziert ffx69.de, http://www.ffx69.de/ --->\n\n\n\n";
/*    if ($banners) {
	include("banners.php");
    } */
    ...
    $tmpl_file = "themes/ffx69_icicle/header.html";
    ...
    print $r_file;
    blocks(left);
    $tmpl_file = "themes/ffx69_icicle/left_center.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
...
function themefooter() {
    ...
    if ($index == 1) {
	$tmpl_file = "themes/ffx69_icicle/center_right.html";
	$thefile = implode("", file($tmpl_file));
	$thefile = addslashes($thefile);
	$thefile = "\$r_file=\"".$thefile."\";";
	eval($thefile);
    ...
    $tmpl_file = "themes/ffx69_icicle/footer.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
...
function themeindex ($aid, $informant, $time, $title, $counter, $topic, $thetext, $notes, $morelink, $topicname, $topicimage, $topictext) {
    ...
    if (($ridaaa1*2) != $loonr) {
	$tmpl_file = "themes/ffx69_icicle/story_home.html";
	$thefile = implode("", file($tmpl_file));
	$thefile = addslashes($thefile);
	$thefile = "\$r_file=\"".$thefile."\";";
	eval($thefile);
...
function themearticle ($aid, $informant, $datetime, $title, $thetext, $topic, $topicname, $topicimage, $topictext) {
    ...
    $tmpl_file = "themes/ffx69_icicle/story_page.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
...
function themesidebox($title, $content) {
    $tmpl_file = "themes/ffx69_icicle/blocks.html";
    $thefile = implode("", file($tmpl_file));
    $thefile = addslashes($thefile);
    $thefile = "\$r_file=\"".$thefile."\";";
    eval($thefile);
