...
function automated_news() {
	...
	if (($date[1] <= $year) AND ($date[2] <= $month) AND ($date[3] <= $day)) {
		...
		while ($row2 = $db->sql_fetchrow($result2)) {
		    ...
		    $sql = "INSERT INTO ".$prefix."_stories VALUES (NULL, '$row2[catid]', '$row2[aid]', '$title', '$row2[time]', '$hometext', '$bodytext', '0', '0', '$row2[topic]', '$row2[informant]', '$notes', '$row2[ihome]', '$row2[alanguage]', '$row2[acomm]', '0', '0', '0', '0')";
		    $db->sql_query($sql);
